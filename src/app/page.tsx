...
// --- AJUSTE PARA SILENCIAR ALERTAS: Tipagem explícita nas importações dinâmicas ---
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Ajuste ícone padrão do leaflet apenas no cliente para evitar alertas de SSR
  ...
  const filteredNodes = nodes.filter(node => {
    if (filter === 'local') {
      return userNode ? node.city === userNode.city : true
    } else if (filter === 'type') {
      return typeFilter === 'All' ? true : node.type === typeFilter
    }
    return true
  })
  ...
  // Renderização condicional para evitar erros de hidratação no Next.js
    ...
    <main style={{ backgroundColor: '#020617', color: 'white', minHeight: '100vh', width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: 'sans-serif', overflowX: 'hidden', position: 'relative' }}>
      ...
      <nav style={{ width: '100%', maxWidth: '1200px', padding: '20px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button onClick={toggleLocale} style={{ padding: '10px 18px', borderRadius: '12px', backgroundColor: '#22d3ee', color: '#020617', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 0 15px #22d3ee' }}>
            {locale === 'pt' ? 'EN' : 'PT'}
          </button>
        </div>
      </nav>
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: '1250px', width: '100%', padding: '60px 40px' }}>
        ...
        <div style={{ marginTop: '120px', padding: '50px', borderRadius: '24px', backgroundColor: '#0b1120', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => setFilter('type')} style={filter==='type'?activeBtnStyle:btnStyle}>Tipo</button>
            {filter==='type' && (
          ...
          <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
            <input value={city} onChange={e => setCity(e.target.value)} placeholder="Enter your city..." style={inputStyle} />
            <button onClick={createNode} style={btnStyle}>Create Digital Node</button>
          </div>

          <div style={{ marginTop: '30px', height: '400px', borderRadius: '16px', overflow: 'hidden' }}>
            {/* Componente agora silenciado com tipagem customizada */}
            <MapContainer center={defaultCenter} zoom={2} style={{ height: '100%', width: '100%' }}>
              ...
              {filteredNodes.map(node => (
                <Marker key={node.id} position={[node.latitude, node.longitude]}>
                  <Popup>{node.city} {node.type ? `- ${node.type}` : ''}</Popup>
                </Marker>
              ))}
              {userNode && (
                <Marker position={[userNode.latitude, userNode.longitude]}>
                  <Popup>Your Node: {userNode.city}</Popup>
                </Marker>
              )}
            </MapContainer>
          </div>
...
// ===== COMPONENTES AUXILIARES COM TIPAGEM PARA VSCODE =====
  ...
  return (
    <div style={{ borderRadius: '24px', overflow: 'hidden', backgroundColor: '#0f172a', border: '1px solid rgba(255,255,255,0.05)' }}>
      ...
      <div style={{ padding: '20px' }}>
        ...
        <a href={url} target="_blank" rel="noopener noreferrer" style={{ color: 'white', textDecoration: 'none', fontSize: '12px', fontWeight: 'bold', border: '1px solid rgba(255,255,255,0.1)', padding: '8px 15px', borderRadius: '8px' }}>Ver Projeto</a>
      </div>
    </div>
  )
}
...
const activeBtnStyle: React.CSSProperties = { ...btnStyle, boxShadow: '0 0 15px #22d3ee', border: '1px solid #22d3ee' }
...
const selectStyle: React.CSSProperties = { padding: '10px', borderRadius: '12px', backgroundColor: '#020617', color: 'white', border: '1px solid rgba(255,255,255,0.1)' }